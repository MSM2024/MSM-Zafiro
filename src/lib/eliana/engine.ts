...
import { ELIANA_SYSTEM_PROMPT } from "./system-prompt"
  ...
  try {
    ...
    const res = await fetch("/api/eliana/chat", {
      ...
      body: JSON.stringify({
        ...
        session: getSession() ? { email: getSession()!.email } : undefined,
      }),
    ...
    const response: ElianaResponse = {
      text: data.text || "No pude procesar tu mensaje. Por favor, intenta reformularlo.",
